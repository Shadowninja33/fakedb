...
router.post('/user/session', (req, res, next) => {
    if (sessionSystem.checkSession(req)) {
        ...
        databaseSystem.getSQLPool().query('SELECT id, password, password_salt, school_user FROM babydriver_users WHERE email = ?', [req.body.email], function(err, result, fields) {
            if (err) {
                next({
                    error_code: 500,
                ...
                if (passwordSystem.verifyPassword(req.body.password, hashed, salt)) {
                    sessionSystem.setupSession(req, result[0].id, req.body.email, result[0].school_user);
                    res.redirect('/user');
                } else {
                    next({
                        error_code: 401,
